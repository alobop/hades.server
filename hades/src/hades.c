...
{
    if (NULL == hades || NULL == config || NULL == config->memory_pool.pool || NULL == config->mutex.lock ||
        NULL == config->mutex.unlock || NULL == config->transport.read_message ||
    ...
    *hades = (hades_t){0};
    ...
    for (size_t i = 0; i < config->request_count; i++)
    {
        ...
        command->associated_response =
            (hades_protocol_response_t*)(((uintptr_t)command->associated_request) + hades->command_size);
...
hades_status_t hades_start(hades_t* hades)
{
    ...
    if (NULL == hades->mutex.lock || NULL == hades->mutex.unlock || NULL == hades->transport.read_message ||
        NULL == hades->commands)
    {
...
hades_status_t hades_register_service(hades_t* hades, hades_service_entry_t* registration_entry, void* context)
{
    ...
    if (NULL == hades->mutex.unlock || NULL == hades->mutex.lock)
    {
        return HADES_E_UNEXPECTED;
    }
...
hades_status_t hades_rpc_complete(hades_rpc_t* rpc, hades_status_t completion_status, void* output_msg)
{
    ...
    *rpc->associated_response =
        (hades_protocol_response_t){.request = 0, .id = rpc->associated_request->id, .status = completion_status};
