...
void hades_request_receive(hades_t* hades, hades_request_t* command)
{
    ...
    hades->transport.read_message(
        hades->transport.transport,
        command->associated_request,
        hades->command_size,
        hades_request_receive_complete,
        command);
    ...
    size_t response_size)
{
    ...
    *command->associated_response =
        (hades_protocol_response_t){.request = 0, .id = command->associated_request->id, .status = completion_status};
    ...
    hades->mutex.lock(hades->mutex.mutex);
    hades->transport.write_message(
        hades->transport.transport, command->associated_response, response_size, hades_request_send_complete, command);
    hades->mutex.unlock(hades->mutex.mutex);
}
